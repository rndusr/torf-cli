...
+
* Display file and other sizes as precise numbers.
* When verifying, corrupt files are always read to the end instead of skipping
  them after the first corrupt piece.

*--json*, *-j*::
Print a JSON object to stdout that contains all the torrent information and
errors.  Progress is not reported.
