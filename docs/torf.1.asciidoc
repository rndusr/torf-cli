...
* *torf* *-i* _INPUT_ +
Display information stored in the torrent file _INPUT_.

* *torf* *-i* _INPUT_ [_OPTIONS_] *-o* _TORRENT_ +
Edit the existing torrent file _INPUT_ (e.g. to fix a typo in the comment) and
...
*--in*, *-i* _INPUT_::
Read metainfo from the torrent file _INPUT_.  Except when verifying, _INPUT_ may
also be a magnet URI.
