...
torrent must have identical name and files.  If _PATH_ is a directory, it is
searched recursively for a matching torrent.  This option may be given multiple
times.

*--noreuse*, *-R*::
Ignore all *--reuse* arguments.  This is particularly useful you have reuse
...
+
Unless *--verbose* is given twice, the "pieces" field in the "info" section is
excluded.
